...
const source = await readFile(resolve(process.cwd(), "contracts/BaseHypeWall.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "BaseHypeWall.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["BaseHypeWall.sol"]?.BaseHypeWall;
...
console.log(`Deploying BaseHypeWall from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_HYPE_WALL_CONTRACT_ADDRESS to this contract address in Vercel.");
